...
Model* Createmodel1()
{
    srand(time(0));
        ...
        return new Model(name,(Players[0]->GetHealth()-50),Players[0]->GetStamina(),Players[0]->GetCurrentWeapon(), Players[0]->GetConsumerItem(),Players[0]->GetBagpack(),(Players[0]->GetSkillFire()+1),Players[0]->GetSkillCold(),Players[0]->GetMoney(),gender,"zombie");
    ...
    {   
        ...
        if (t < 80)
        {
            ...
            if (Enemy ==1)
            {
                ...
                Model* EnemyZombie = zombie.Createmodel1();
                ...
                while (true)
                {
                    if(random == 0)
                    {
                        ...
                        if (EnemyZombie->GetHealth() <= 0)
                        {
                            ...
                            {
                                //HumanCharacter Player = *players[i];
                                players[i]->ManageMoney(0,20);
                                players[i]->IncreasePoint(50);
                                players[i]->Injury(0,200);
                                players[i]->Activity(0,100);
                                if (players[i]->GetPoint()%100==0)
                                players[i]->IncreaseLevel(1);
